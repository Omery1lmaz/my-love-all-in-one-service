...
const updateUserFavoriteMovieExpressValidator = [
    (0, express_validator_1.body)("favoriteMovie")
        .isObject()
        .withMessage("favoriteMovie bir obje olmalıdır")
        .custom((value) => {
        if (!value.id)
            throw new Error("Film ID'si gereklidir");
        if (!value.title)
            throw new Error("Film başlığı gereklidir");
        if (!value.poster_path)
            throw new Error("Film poster yolu gereklidir");
        if (!value.release_date)
            throw new Error("Film yayın tarihi gereklidir");
...
exports.default = updateUserFavoriteMovieExpressValidator;
