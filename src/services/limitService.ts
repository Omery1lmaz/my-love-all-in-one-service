...
export class LimitService {
  ...
  static async checkLimit(
    ...
    try {
      ...
      const userSubscription = await Subscription.getDefaultSubscription(
        new mongoose.Types.ObjectId(userId)
      );

      // Plan tipine göre limit belirle
      const limit = limitConfig[userSubscription.planType as keyof LimitConfig];
      
      // Limit kontrolü
      if (currentCount >= limit) {
        // AdSense kullanabilirlik kontrolü
        const adsenseAvailable = await this.checkAdSenseAvailability(userId);
   ...
   */
  private static async checkAdSenseAvailability(userId: string): Promise<boolean> {
    try {
   ...
   */
  static async recordAdSenseUsage(userId: string, operationName: string): Promise<void> {
    try {
      const user = await User.findById(userId);
   ...
   */
  static createLimitExceededResponse(limitResult: LimitCheckResult, operationName: string) {
    const response: any = {
