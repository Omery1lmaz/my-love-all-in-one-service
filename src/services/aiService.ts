...
export class AIService {
  ...
  async generateInsights(healthScoreData: any, additionalData?: any): Promise<any[]> {
    try {
      const prompt = `
      ...
      Ek veriler:
      - Mesaj sayısı: ${healthScoreData.dataPoints?.messageCount || 0}
      - Ortalama yanıt süresi: ${healthScoreData.dataPoints?.responseTime || 0} dakika
      - Pozitif sentiment: ${healthScoreData.dataPoints?.positiveSentiment || 0}%
      - Aktivite katılımı: ${healthScoreData.dataPoints?.activityEngagement || 0}
      - Çatışma sayısı: ${healthScoreData.dataPoints?.conflictCount || 0}
  ...
  async analyzeSentiment(messages: any[]): Promise<{ positive: number; neutral: number; negative: number }> {
    try {
  ...
  async generateRecommendations(healthScore: any, trends: any): Promise<{
    immediate: string[];
    shortTerm: string[];
    longTerm: string[];
  }> {
  ...
  async analyzeCommunicationPatterns(messages: any[]): Promise<any> {
    try {
      if (messages.length === 0) {
        return {
  ...
  private simpleSentimentAnalysis(messages: any[]): { positive: number; neutral: number; negative: number } {
