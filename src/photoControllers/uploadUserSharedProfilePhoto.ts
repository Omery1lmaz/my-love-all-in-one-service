...
export const uploadUserSharedProfilePhotoController = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    try {
      ...
      user.sharedProfilePic = newPhoto._id as any;
      await user.save();
      if (user.partnerId) {
        const partner = await User.findById(user.partnerId);
        if (partner) {
          partner.sharedProfilePic = newPhoto._id as any;
          await partner.save();
        }
      }
      res.status(201).json({
...
export default uploadUserSharedProfilePhotoController;
